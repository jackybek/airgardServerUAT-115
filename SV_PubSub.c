...
// tested okay with v1.3rc1-193 open62541.o
...
static void addPubSubConnection(UA_Server *, char *, char *);	//UA_NetworkAddressUrlDataType *);
...
static void
addPubSubConnection(UA_Server *uaServer, char* addressUrl, char* networkInterface)
...
/*
defined in ~/open62541/include/open62541/server_pubsub.h
...
*/
    #ifdef DEBUG_MODE
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "CreateServerPubSubSV_PubSub.c : In addPubSubConnection() \n");
    #endif

    UA_PubSubConnectionConfig PubSubConnectionConfig;
    memset(&PubSubConnectionConfig, 0, sizeof(PubSubConnectionConfig));

    /*1*/PubSubConnectionConfig.name = UA_STRING(CONNECTION_CONFIGNAME);	// UA_STRING("Publisher Connection")
    /*2*/PubSubConnectionConfig.enabled = UA_TRUE;
    /*3*/PubSubConnectionConfig.publisherIdType = UA_PUBLISHERIDTYPE_UINT16;
    /*4*/PubSubConnectionConfig.publisherId.uint16 = PUBLISHERID;	// Changed to static publisherId from random generation to identify the publisher on subscriber side

    // UA_NetworkAddressUrlDataType is defined in ~/open62541/build/src_generated/open62541/types_generated.h
/*
typedef struct {
    UA_String networkInterface;
    UA_String url;
...
*/
    /* for testing only */ UA_NetworkAddressUrlDataType networkAddressUrl = {UA_STRING_NULL, UA_STRING_NULL};
		networkAddressUrl.networkInterface = UA_STRING(networkInterface);
		networkAddressUrl.url = UA_STRING(addressUrl);

    /*6*/UA_Variant_setScalar(&PubSubConnectionConfig.address, &networkAddressUrl, &UA_TYPES[UA_TYPES_NETWORKADDRESSURLDATATYPE]);

printf("SV_PubSub.c : in addPubSubConnection() : parameters passed in are \n");
printf("              passed in <networkInterface>	  : %s \n", networkInterface);
printf("	      passed in <addressUrl> 		  : %s \n", addressUrl);
...
printf("       checking PubSubConnectionConfig.address (type VARIANT) \n");
                        UA_NetworkAddressUrlDataType ConnConfig_raw_data_str = *(UA_NetworkAddressUrlDataType *)PubSubConnectionConfig.address.data;
                        char mynetworkInterface[128];
                        strcpy(mynetworkInterface, ConnConfig_raw_data_str.networkInterface.data);
printf("                address.networkInterface 	  : %s \n", mynetworkInterface);
                        char myurl[128];
                        strcpy(myurl, ConnConfig_raw_data_str.url.data);
printf("                address.url			  : %s \n", myurl);
// this above did not copy contents in networkAddressUrl to PubSubConnectionConfig.address
...
*/
    ...
    {
        /*5*/PubSubConnectionConfig.transportProfileUri = UA_STRING(TRANSPORT_PROFILE_URI_MQTT); //*transportProfile;
	/*
	UA_ServerConfig *config = UA_Server_getConfig(uaServer);
    	/*7*/
...
// line 236
	...
	switch (MQTT_Port)
	{
		case 1883 : MQTT_TLS_Enable = UA_FALSE; connectionOptionsCount = 3; break;
		case 1884 : MQTT_TLS_Enable = UA_FALSE; connectionOptionsCount = 5; break;
		case 8883 : MQTT_TLS_Enable = UA_TRUE; connectionOptionsCount = 6; break;
		case 8884 : MQTT_TLS_Enable = UA_TRUE; connectionOptionsCount = 10; break;
	}
	...
	else
	{
		...
		else // MQTT_Port = 1883 || MQTT_Port == 8883
			connectionOptionsCount = 3;
	}
	*/
 ...
 */
		...
		if ( (MQTT_Port == 8883) || (MQTT_Port == 8884) )
		{
			...
			connectionOptions[connectionOptionsIndex].key = UA_QUALIFIEDNAME(namespaceIndex, "mqttCaFilePath");	//MQTT_CA_FILE_PATH_OPTION_NAME);	// "mqttCaFilePath");
	                UA_String mqttCaFilePath = UA_STRING(CA_FILE_PATH_MQTT);
        	        UA_Variant_setScalar(&connectionOptions[connectionOptionsIndex].value, &mqttCaFilePath, &UA_TYPES[UA_TYPES_STRING]);
		}
		if ( MQTT_Port == 8884 )
		{
   	...
   	PubSubConnectionConfig.connectionProperties = connectionOptions;
    	PubSubConnectionConfig.connectionPropertiesSize = connectionOptionsIndex+1;	// add 1 because the index start at 0;
	...
	int index=0;
        ...
        printf("PubSubConnectionConfig.name                                           : %s \n", PubSubConnectionConfig.name.data);
        printf("PubSubConnectionConfig.enabled                                        : %d \n", PubSubConnectionConfig.enabled);
        printf("PubSubConnectionConfig.PublisherIdType                                : %d \n", PubSubConnectionConfig.publisherIdType);
        printf("PubSubConnectionConfig.PublisherId                                    : %d \n", PubSubConnectionConfig.publisherId.uint16);
        printf("PubSubConnectionConfig.transportProfileUri                            : %s \n", PubSubConnectionConfig.transportProfileUri.data);
		// connectionConfig.address is of type UA_Variant
	//	UA_String raw_data_str = *(UA_String *)PubSubConnectionConfig.address.data;
	//printf("PubSubConnectionConfig.address			      		      : %s \n", raw_data_str.data);
                        UA_NetworkAddressUrlDataType ConnConfig_raw_data_str = *(UA_NetworkAddressUrlDataType *)PubSubConnectionConfig.address.data;
                        char mynetworkInterface[128];
                        strcpy(mynetworkInterface, ConnConfig_raw_data_str.networkInterface.data);
	printf("PubSubConnectionConfig.address.networkInterface          	      : %s \n", mynetworkInterface);
                        char myurl[128];
                        strcpy(myurl, ConnConfig_raw_data_str.url.data);
	printf("PubSubConnectionConfig.address.url                       	      : %s \n", myurl);

        //printf("connectionConfig.connectionTransportSettings                  : %d \n", *(UA_Int16 *)connectionConfig.connectionTransportSettings.data); // variant
        printf("PubSubConnectionConfig.connectionPropertiesSize                       : %d \n", PubSubConnectionConfig.connectionPropertiesSize);
        printf("---------------------------------------------------------------------\n");
        printf("%d. PubSubConnectionConfig.connectionProperties[%d].key : mqttClientId  : %s \n", index,index,PubSubConnectionConfig.connectionProperties[index].key.name.data); // [0].key.name is of t$
		//connectionConfig.connectionProperties[0].value is of type UA_Variant
		UA_String raw_data_str = *(UA_String *)PubSubConnectionConfig.connectionProperties[index].value.data;
        printf("%d. PubSubConnectionConfig.connectionProperties[%d].value (UA_String)   : %s \n", index,index,raw_data_str.data);  // [$

	index++;	// index now is 1
        printf("%d. PubSubConnectionConfig.connectionProperties[%d].key : sendBufferSize: %s \n", index,index,PubSubConnectionConfig.connectionProperties[index].key.name.data);
		//connectionConfig.connectionProperties[1].value is of type UA_Variant
		UA_Int16 raw_data_int16 = *(UA_Int16 *)PubSubConnectionConfig.connectionProperties[index].value.data;
        printf("%d. PubSubConnectionConfig.connectionProperties[%d].value (UInt16)      : %d \n", index,index,raw_data_int16);

	index++;	// index now is 2
        printf("%d. PubSubConnectionConfig.connectionProperties[%d].key : recvBufferSize: %s \n", index,index,PubSubConnectionConfig.connectionProperties[index].key.name.data);
		         raw_data_int16 = *(UA_Int16 *)PubSubConnectionConfig.connectionProperties[index].value.data;
        printf("%d. PubSubConnectionConfig.connectionProperties[%d].value (UInt16)      : %d \n", index,index,raw_data_int16);

	if ( (MQTT_Port == 1884) || (MQTT_Port == 8884) )
	{
		index++;	// index now is 3
        	printf("%d. PubSubConnectionConfig.connectionProperties[%d].key : mqttUsername  : %s \n", index,index,PubSubConnectionConfig.connectionProperties[index].key.name.data);
		raw_data_str = *(UA_String *)PubSubConnectionConfig.connectionProperties[index].value.data;
	        printf("%d. PubSubConnectionConfig.connectionProperties[%d].value (UA_String)   : %s \n", index,index,raw_data_str.data);

		index++;	// index now is 4
        	printf("%d. PubSubConnectionConfig.connectionProperties[%d].key : mqttPassword  : %s \n", index,index,PubSubConnectionConfig.connectionProperties[index].key.name.data);
		raw_data_str = *(UA_String *)PubSubConnectionConfig.connectionProperties[index].value.data;
	        printf("%d. PubSubConnectionConfig.connectionProperties[%d].value (UA_String)   : %s \n", index,index,raw_data_str.data);
	}
	if ( (MQTT_Port == 8883) || (MQTT_Port == 8884) ) //(MQTT_TLS_Enable == UA_TRUE)
	{
		index++;	// index now is 5
        	printf("%d. PubSubConnectionConfig.connectionProperties[%d].key : mqttUseTLS    : %s \n", index,index,PubSubConnectionConfig.connectionProperties[index].key.name.data);
			raw_data_int16 = *(UA_Boolean *)PubSubConnectionConfig.connectionProperties[index].value.data;
        	printf("%d. PubSubConnectionConfig.connectionProperties[%d].value (UA_Boolean)  : %d \n", index,index,raw_data_int16);

		index++;
		printf("%d. PubSubConnectionConfig.connectionProperties[%d].key : mqttCaPath    : %s \n", index,index,PubSubConnectionConfig.connectionProperties[index].key.name.data);
			raw_data_str = *(UA_String *)PubSubConnectionConfig.connectionProperties[index].value.data;
        	printf("%d. PubSubConnectionConfig.connectionProperties[%d].value (UA_String)   : %s \n", index,index,raw_data_str.data);

		index++;
         	printf("%d. PubSubConnectionConfig.connectionProperties[%d].key : mqttCaFilePath: %s \n", index,index,PubSubConnectionConfig.connectionProperties[index].key.name.data);
			raw_data_str = *(UA_String *)PubSubConnectionConfig.connectionProperties[index].value.data;
			char caFilePath[128];
			strcpy(caFilePath, raw_data_str.data);
        	printf("%d. PubSubConnectionConfig.connectionProperties[%d].value (UA_String)   : %s %s\n", index,index,raw_data_str.data, caFilePath);
	}
	if (MQTT_Port == 8884)
	{
		index++;
        	printf("%d. PubSubConnectionConfig.connectionProperties[%d].key : mqttClientCertPath : %s \n", index,index,PubSubConnectionConfig.connectionProperties[index].key.name.data);
			raw_data_str = *(UA_String *)PubSubConnectionConfig.connectionProperties[index].value.data;
			char clientCertPath[128];
			strcpy(clientCertPath, raw_data_str.data);
        	printf("%d. PubSubConnectionConfig.connectionProperties[%d].value (UA_String)        : %s %s\n", index,index,raw_data_str.data, clientCertPath);

		index++;
        	printf("%d. PubSubConnectionConfig.connectionProperties[%d].key : mqttClientKeyPath  : %s \n", index,index,PubSubConnectionConfig.connectionProperties[index].key.name.data);
			raw_data_str = *(UA_String *)PubSubConnectionConfig.connectionProperties[index].value.data;
			char clientKeyPath[128];
			strcpy(clientKeyPath, raw_data_str.data);
        	printf("%d. PubSubConnectionConfig.connectionProperties[%d].value (UA_String)        : %s %s\n", index,index,raw_data_str.data, clientKeyPath);
	...
	// initiatise the value of PubSubconnectionIdentifier (UA_NodeId) so that we can check later in addSubscription()
    	...
    	UA_StatusCode retval = UA_Server_addPubSubConnection(uaServer, &PubSubConnectionConfig, &PubSubconnectionIdentifier);
    ...
    else if (MQTT_Enable == UA_FALSE && AMQP_Enable == UA_TRUE)
    {
        ...
        /*5*/PubSubConnectionConfig.transportProfileUri = UA_STRING(TRANSPORT_PROFILE_URI_AMQP); //transportProfile;
    ...
    PubSubConnectionConfig.name = UA_STRING("UDP-UADP Connection 1");
    PubSubConnectionConfig.enabled = UA_TRUE;
    /*5*/PubSubConnectionConfig.transportProfileUri = UA_STRING(TRANSPORT_PROFILE_URI_UDP); //*transportProfile;
    //PubSubConnectionConfig.transportProfileUri = UA_STRING("http://opcfoundation.org/UA-Profile/Transport/pubsub-udp-uadp");
    /* The address and interface is part of the standard
     * defined UA_NetworkAddressUrlDataType. */
    UA_NetworkAddressUrlDataType networkAddressUrl = {UA_STRING_NULL , UA_STRING("opc.udp://224.0.0.22:4840/")};
    UA_Variant_setScalar(&PubSubConnectionConfig.address, &networkAddressUrl, &UA_TYPES[UA_TYPES_NETWORKADDRESSURLDATATYPE]);
    PubSubConnectionConfig.publisherId.uint16 = PUBLISHERID;
	...
	*/
	...
	connectionOptionsIndex++;
    	...
    	PubSubConnectionConfig.connectionProperties = connectionProperties;
	PubSubConnectionConfig.connectionPropertiesSize = connectionOptionsIndex;
	...
	*/
	...
	printf("PubSubConnectionConfig.PublisherId				: %d \n", PubSubConnectionConfig.publisherId.uint16);
        printf("PubSubConnectionConfig.transportProfileUri            		: %s \n", PubSubConnectionConfig.transportProfileUri.data);
        printf("PubSubConnectionConfig.address                        		: %s \n", PubSubConnectionConfig.address.data);
	//printf("PubSubConnectionConfig.connectionTransportSettings			: %d \n", *(UA_Int16 *)connectionConfig.connectionTransportSettings.data); // variant
	printf("PubSubConnectionConfig.connectionPropertiesSize			: %d \n", PubSubConnectionConfig.connectionPropertiesSize);
        printf("---------------------------------------------------------------------\n");
	printf("PubSubConnectionConfig.connectionProperties[0].key   (string)		: %s \n", PubSubConnectionConfig.connectionProperties[0].key.name.data); // [0].key.name is of type UA_STRING
	//printf("PubSubConnectionConfig.connectionProperties[0].value (int16)		: %d \n", *(UA_Int16 *)connectionConfig.connectionProperties[0].value.data);  // [0].value is a variant;
        printf("PubSubConnectionConfig.connectionProperties[1].key   (string) 	: %s \n", PubSubConnectionConfig.connectionProperties[1].key.name.data);
        //printf("PubSubConnectionConfig.connectionProperties[1].value (boolean) 	: %d \n", *(UA_Boolean *)connectionConfig.connectionProperties[1].value.data);
        printf("PubSubConnectionConfig.connectionProperties[2].key   (string) 	: %s \n", PubSubConnectionConfig.connectionProperties[2].key.name.data);
        //printf("PubSubConnectionConfig.connectionProperties[2].value (boolean)	: %d \n", *(UA_Boolean *)connectionConfig.connectionProperties[2].value.data);
        printf("---------------------------------------------------------------------\n");
        printf("networkAddressUrl.networkInterface              		: %s \n", networkAddressUrl.networkInterface.data);
        printf("networkAddressUrl.url                          	 		: %s \n", networkAddressUrl.url.data);
	#endif
	//sleep(5);
        ...
        UA_StatusCode retval = UA_Server_addPubSubConnection(uaServer, &PubSubConnectionConfig, &connectionIdentifier);
        if (retval == UA_STATUSCODE_GOOD)
        {
                UA_String output;
                UA_String_init(&output);
...
addWriterGroup(UA_Server *uaServer)
{
    ...
    writerGroupConfig.enabled = UA_FALSE;	// according to tutorial : tutorial_pubsub_mqtt_publish.c
    ...
    /* decide whether to use JSON or UADP encoding, encryption mode cannot use JSON*/
    if(useJson && MQTT_Enable && !MQTT_TLS_Enable)
    {
	#ifdef DEBUG_MODE
	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "SV_PubSub.c :addWriterGroup : useJson = UA_TRUE");
        ...
        // Add the encryption key information - KIV because 1883 fail to publish
        //UA_ServerConfig *config = UA_Server_getConfig(uaServer);
	...
	// the following is not in tutorial: tutorial_pubsub_mqtt_publish.c but is in 192.168.1.33
    ...
    else if (useJson && MQTT_Enable && MQTT_TLS_Enable)
    {
	UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "SV_PubSub.c :addWriterGroup : Json Encoding cannot be encrypted over MQTT");
	exit(EXIT_FAILURE);
    }
    else if (!useJson && !MQTT_Enable && !MQTT_TLS_Enable)
    {
	...
	// the following is not in tutorial : tutorial_pubsub_mqtt_publish.c
	UA_Server_addWriterGroup(uaServer, PubSubconnectionIdentifier, &writerGroupConfig, &writerGroupIdentifier);
	UA_Server_setWriterGroupOperational(uaServer, writerGroupIdentifier);
...
*/
	...
	//addPubSubConnection(uaServer, &networkAddressUrl);
	addPubSubConnection(uaServer, networkAddressUrl.url.data, networkAddressUrl.networkInterface.data);
	...
	if (strlen(mode)!=0)
	{
		if ( (strncmp(mode, "nomode", 6)==0) && (strlen(mode)==6) && (brokeraddress == NULL) )
		{
	                ...
	                UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"CreateServerPubSub : successfully initialise Publisher routine (1)");
        	        return;
		}
        	...
        	else if ( (strncmp(mode, "pub", 3)==0) && (strlen(mode)==3) && (brokeraddress != NULL) )
		{
			...
			*/
			UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"CreateServerPubSub : successfully initialise Publisher routine (2)");
			return;
		}
		...
		else if ( (strncmp(mode, "sub", 3)==0) && (strlen(mode)==3) && (brokeraddress != NULL) )
		{
	                ...
	                if(connection != NULL)
			{
				...
				*/
				...
				printf("connection->config->publisherId->numeric : %d\n", connection->config->publisherId.uint16); //should not be NULL
